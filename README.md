...
  <style>
    /* E-NAPLÓ VILÁGOS TÉMA VÁLTOZÓI */
    :root {
      --bg: #f4f6f8;        /* Világos háttér (szürkésfehér) */
      --card: #ffffff;      /* Fehér kártya/konténer */
      --text: #333333;      /* Sötét szöveg */
      --muted: #777777;     /* Halvány (szürke) */
      --accent: #007bff;    /* Elsődleges kék (mint a legtöbb e-naplóban) */
      --accent-2: #28a745;  /* Zöld (mentéshez) */
      --danger: #dc3545;    /* Piros (törléshez) */
      --border: #dddddd;    /* Világos szegély */
      --cell: #ffffff;      /* Cella háttér (fehér) */
      --cell-focus: #f0f8ff; /* Fókusz (nagyon világos kék) */
      --header-bg: #e9ecef; /* Táblázatfejléc (világosszürke) */
    }
    * { box-sizing: border-box; }
    body {
      margin: 0; padding: 2rem; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;
      background: var(--bg); /* Egyszerű világos háttér */
      color: var(--text);
    }
    h1 { margin: 0 0 0.75rem; font-size: 1.8rem; color: var(--accent); } /* Kék cím */
    p { margin: 0 0 1rem; color: var(--muted); }
    .wrap {
      max-width: 1100px; margin: 0 auto; background: var(--card);
      border: 1px solid var(--border); border-radius: 8px; /* Kisebb lekerekítés */
      padding: 1.5rem; /* Nagyobb belső tér */
      box-shadow: 0 5px 15px rgba(0,0,0,0.1); /* Világosabb árnyék */
    }
    .controls {
      display: flex; flex-wrap: wrap; gap: 0.75rem; /* Nagyobb rés a gombok között */
      align-items: center; margin-bottom: 1rem;
    }
    /* Általános gomb/select stílus */
    button, select {
      background: var(--card); color: var(--text); border: 1px solid var(--border);
      padding: 0.6rem 1rem; border-radius: 5px; /* Kisebb lekerekítés */
      cursor: pointer; font-weight: 500;
      transition: background 0.2s;
      appearance: none;
    }
    button:hover { background: #f0f0f0; } /* Halvány hover */
    select { padding-right: 2rem; } /* Hely a lenyíló nyílnak */

    /* SAVE gomb (Zöld) */
    .save { border-color: var(--accent-2); background: var(--accent-2); color: #fff; font-weight: 600; }
    .save:hover { background: #1e7e34; }

    /* CLEAR gomb (Piros) */
    .clear { border-color: var(--danger); background: var(--danger); color: #fff; font-weight: 600; }
    .clear:hover { background: #c82333; }

    /* REDIRECT gomb (Kék) */
    .redirect { border-color: var(--accent); background: var(--accent); color: #fff; font-weight: 600; }
    .redirect:hover { background: #0056b3; }
    
...
      padding: 0.75rem 0.6rem; border-bottom: 1px solid var(--border); font-size: 1rem;
...
    }
    tbody th {
      text-align: center; /* Óraszám középre */
      padding: 0.45rem; width: 6ch; color: var(--muted); background: #f8f9fa; /* Nagyon világos szürke */
...
    }
    .cell {
      width: 100%; min-height: 4rem; /* Nagyobb magasság a celláknak */
      padding: 0.6rem; border: 0; background: transparent; color: var(--text);
      font: inherit; outline: none; resize: none; display: block;
    }
    .cell:focus { background: var(--cell-focus); }
    .grid { overflow: auto; max-height: 65vh; border-radius: 8px; }
    .footer {
...
      <button class="redirect" id="redirectBtn">Osztálynapló</button>
...
    <div class="footer">
      <span>Tipp: Enter a sor végén, Ctrl+S mentés.</span>
...
    // --- CÉL URL ---
    const REDIRECT_URL = "https://sites.google.com/view/osztlynaplonline/oszt%C3%A1lynapl%C3%B3/napl%C3%B3"; 
    // -------------------
...
      // --- ÚJ ESEMÉNYKEZELŐ AZ ÁTIRÁNYÍTÓ GOMHOZ ---
      document.getElementById("redirectBtn").addEventListener("click", () => {
          // Átirányítás a beállított URL-re
